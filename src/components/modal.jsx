...
function Modal({complete}) {
  const { register, handleSubmit} = useForm();

    async function onAddRevenue(data) {
      await api.put(`/revenues/${data.id}`, data)
    }
  return (
      ...
      <div className="flex flex-col bg-white w-[50%] lg:w[25%] p-6 rounded-lg shadow-lg space-y-4">
        ...
        <form action={() => handleSubmit(onAddRevenue)()} className="flex flex-col  space-y-3">
                <section className="flex flex-col items-start">
                  ...
                  <SearchBar placeholder="Member" type="text" id="member" {...register("member")} value={complete.member}/>
                </section>
                
                <section className="flex flex-row gap-4 w-full">
                  <div className="flex flex-col items-start w-full">
                    <label htmlFor="type" className="text-xs">Type</label>
                    <select id="type" className="w-full text-xs bg-gray-100 border rounded-md border-gray-100 hover:cursor-auto focus:border-gray-400 focus:outline-none placeholder:text-gray-500 focus:ring-gray-400 px-2 py-2" {...register("type")} value={complete.type}>
                  ...
                  <div className="flex flex-col items-start w-full">
                    ...
                    <SearchBar placeholder="00,0" type="number"  id="value" {...register("value")} value={complete.value}/>
                  </div>
                </section >
                <section className="flex flex-row gap-4 w-full">
                  <div className="flex flex-col items-start w-full">
                    <label htmlFor="payment" className="text-xs">Payment</label>
                    <select id="payment" className="w-full text-xs bg-gray-100 border rounded-md border-gray-100 hover:cursor-auto focus:border-gray-400 focus:outline-none placeholder:text-gray-500 focus:ring-gray-400 px-2 py-2" {...register("payment")} value={complete.payment} >
                  ...
                  <div className="flex flex-col items-start w-full">
                    ...
                    <SearchBar type="date"  id="date" {...register("date")} value={complete.date} />
                  </div>
                </section >
